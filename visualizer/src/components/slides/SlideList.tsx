...
function Marker({ style, index }: { style: ListContent["style"]; index: number }) {
  ...
  if (style === "check") {
    return (
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/25 text-base text-white/50">
        {index + 1}
      </span>
    );
  }
  return <span className="mt-3.5 h-2.5 w-2.5 shrink-0 rounded-full bg-white/40" />;
}

export function SlideList({ content, revealCount }: { content: ListContent; revealCount: number }) {
  return (
    <div className="w-full max-w-4xl">
      <h1 className="font-display mb-2 text-5xl font-bold text-white">{content.heading}</h1>
      {content.subheading && <p className="mb-8 text-xl text-white/50">{content.subheading}</p>}
        ...
        {content.items.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-2xl leading-snug text-white/90">{item}</span>
            </motion.li>
          );
        })}
      </ul>
