        ...
        {PATHS.map((row) => (
          <div
            ...
            className={`flex items-center justify-between rounded-xl border px-6 py-5 ${
              row.match ? "border-flamingo/50 bg-flamingo/10" : "border-white/10 bg-white/[0.02] opacity-40"
            }`}
          >
            <code className="text-lg text-white/80">{row.path}</code>
            <span className="text-white/30">→</span>
            <code className={row.match ? "text-flamingo" : "text-white/40"}>{row.loads}</code>
          </div>
        ))}
      </div>
      <p className="text-base text-white/40">Editing a file under <code>src/api/</code> — only the matching guide loads.</p>
    </div>
  );
}
