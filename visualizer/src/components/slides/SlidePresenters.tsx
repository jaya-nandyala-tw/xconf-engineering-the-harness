...
function PhotoSlot({ photo, accent }: { photo?: string; accent: ReturnType<typeof accentForIndex> }) {
  ...
  return (
    <div
      className={`flex h-28 w-28 shrink-0 flex-col items-center justify-center gap-1 rounded-full border-2 border-dashed bg-ink/[0.03] text-ink/30 ${ACCENT_BORDER[accent]}`}
    >
...
export function SlidePresenters({ content }: { content: PresentersContent }) {
  return (
    <div className="flex w-full max-w-6xl flex-col items-center gap-10 text-center">
      <p className="text-base uppercase tracking-[0.3em] text-ink/40">Your presenters</p>
        ...
        {content.people.map((person, i) => {
          ...
          return (
            <motion.div
              ...
              className="relative flex items-center gap-6 overflow-hidden rounded-2xl border border-ink/10 bg-ink/[0.03] p-8 text-left"
            >
              <span className={`absolute inset-x-0 top-0 h-1 ${ACCENT_BG[accent]}`} />
              <PhotoSlot photo={person.photo} accent={accent} />
              <div className="min-w-0 flex-1">
                <p className="font-display text-2xl font-bold leading-tight text-ink">{person.name}</p>
                <p className="mt-1 text-base text-ink/50">{person.title}</p>
                <div className="my-3 h-px w-full bg-ink/10" />
                <p className="text-base leading-relaxed text-ink/70">{person.bio}</p>
              </div>
            </motion.div>
          );
        })}
