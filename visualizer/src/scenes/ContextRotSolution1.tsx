...
function Segment({ turn, tone, isSignal }: { turn: Turn; tone: keyof typeof TONE_STYLE; isSignal: boolean }) {
...
const ROLE_STYLE: Record<Role, { wrap: string; label: string; mono?: boolean }> = {
  ...
  subagent: { wrap: "border-violet-400/30 bg-violet-400/10 text-violet-200", label: "sub-agent" },
};
...
function SubAgentPanel({ state }: { state: "shown" | "collapsed" }) {
  return (
    <AnimatePresence mode="wait">
      {state === "shown" ? (
        ...
        >
          <div className="mb-2 flex items-center justify-between text-xs uppercase tracking-wide text-violet-300/70">
        ...
        <motion.div
          ...
          className="rounded-lg border border-violet-400/30 bg-violet-400/10 px-6 py-3 text-lg text-violet-200"
        >
  ...
  return (
    ...
    >
      <div className="text-center">
        <p className="font-mono text-2xl font-semibold text-white/70 line-through decoration-red-400/60">
          {EXPLORATION_TOKENS.toLocaleString()}
        </p>
        <p className="text-xs uppercase tracking-wide text-white/40">without sub-agent</p>
      </div>
      <span className="text-2xl text-emerald-300">→</span>
      <div className="text-center">
        <p className="font-mono text-2xl font-semibold text-emerald-300">{TURNS.summary.tokens}</p>
        <p className="text-xs uppercase tracking-wide text-emerald-300/70">with sub-agent</p>
      </div>
      <div className="ml-2 border-l border-emerald-400/20 pl-6 text-left">
        <p className="font-mono text-3xl font-semibold text-emerald-300">{SUBAGENT_SAVINGS_PCT}%</p>
        <p className="text-sm text-emerald-300/80">less research ever touches the main thread</p>
      </div>
    </motion.div>
  );
}
    ...
    >
      <div className="flex h-full min-h-0 w-full max-w-4xl flex-col items-center justify-center gap-6">
        ...
        {(config.subagentPanel || config.subagentPayoff) && (
          <div className="flex w-full flex-col items-center gap-5">
            {config.subagentPanel && <SubAgentPanel state={config.subagentPanel} />}
            {config.subagentPayoff && <SubAgentPayoff />}
          </div>
        )}
      </div>
    </SceneChrome>
